...
%global npm_bare      go-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-go-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Go configuration providing standardized golangci-lint, staticcheck, and revive rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/go-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/go-config is a shareable configuration preset with no runtime
...
install -pm 0644 .golangci.yml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 staticcheck.conf %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
