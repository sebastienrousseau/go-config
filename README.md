...
<p align="center">
  <img src="./go-config.svg" alt="go-config logo" width="128" />
</p>

<h1 align="center">@sebastienrousseau/go-config</h1>
...
<p align="center">
  <a href="https://github.com/sebastienrousseau/go-config/actions"><img src="https://img.shields.io/github/actions/workflow/status/sebastienrousseau/go-config/ci.yml?branch=main&style=for-the-badge&logo=github" alt="Build Status" /></a>
  <a href="https://www.npmjs.com/package/@sebastienrousseau/go-config"><img src="https://img.shields.io/npm/v/%40sebastienrousseau%2Fgo-config.svg?style=for-the-badge&color=fc8d62&logo=npm" alt="npm package" /></a>
  <a href="https://sebastienrousseau.com/go-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
  <a href="https://scorecard.dev/viewer/?uri=github.com/sebastienrousseau/go-config"><img src="https://img.shields.io/ossf-scorecard/github.com/sebastienrousseau/go-config?style=for-the-badge&label=OpenSSF%20Scorecard&logo=openssf" alt="OpenSSF Scorecard" /></a>
...
- [Exhaustive golangci-lint & staticcheck Suite](#exhaustive-golangci-lint-staticcheck-suite) — before and after rule comparison
...
Install `@sebastienrousseau/go-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/go-config

# pnpm
pnpm add -D @sebastienrousseau/go-config

# yarn
yarn add -D @sebastienrousseau/go-config

# bun
bun add -d @sebastienrousseau/go-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/go-config.git
cd go-config
make            # check + test
```
...
- **Target environment**: Optimized for Go 1.22+ / golangci-lint.
...
```bash
cp node_modules/@sebastienrousseau/go-config/.golangci.yml ./.golangci.yml
cp node_modules/@sebastienrousseau/go-config/staticcheck.conf ./staticcheck.conf
```

### Or point the tool at the installed file

```bash
golangci-lint run --config node_modules/@sebastienrousseau/go-config/.golangci.yml
```
...
`@sebastienrousseau/go-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/go-config` solves this with three deliberate engineering choices:
...
## Exhaustive golangci-lint & staticcheck Suite

Configures 40+ high-precision Go linters with zero false-positives, enforcing error handling, concurrency safety, and struct alignment.
...
### After (@sebastienrousseau/go-config Enforced)
...
When building legacy Go < 1.18 packages that cannot parse generic type parameters or modern linter syntax.
